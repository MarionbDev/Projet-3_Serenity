...
const sendEmailAndChangePasswordDoctor = (req, res) => {
  ...
  const options = {
    ...
    html: `<h2 style="font-size: 32px">Bonjour ${firstname}</h2><p>Votre espace praticien a bien été crée au sein de notre structure.</p><p>Votre identifiant : ${mail}</p><p>Votre mot de passe provisoire est votre prénom (sans majuscule)</p><p>Pour modifier votre mot de passe et sécuriser votre compte, cliquez sur le lien suivant : </p> <a href=${process.env.FRONTEND_URL}/changer-mon-mot-de-passe>Cliquez ici</a>`,
  };
...
const sendEmailAndChangePasswordPatient = (req, res) => {
  const { mail, firstname } = req.body;
...
module.exports = {
  sendEmailAndChangePasswordDoctor,
  sendEmailAndChangePasswordPatient,
};
